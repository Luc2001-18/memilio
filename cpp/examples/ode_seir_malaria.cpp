...
int main()
{
   ...
   // ScalarType total_population                                                        = 100000;
    ...
    model.populations[{mio::AgeGroup(3), mio::oseirvector::InfectionState::Susceptible_vector}] = 1.0 - prop_vector_infected;
  ...
  //  contact_matrix[0].add_damping(0.7, mio::SimulationTime<ScalarType>(30.));
    ...
    //  PATCH-SPECIFIC OVERRIDES
        ...
        auto set_patch_population = [&](mio::oseirvector::Model<ScalarType>& patch,
                                    ScalarType total_humans,
   ...
   for (size_t i = 0; i < 3; ++i) {
        indices_save_edges[0].emplace_back(
            model.populations.get_flat_index({mio::AgeGroup(i), mio::oseirvector::InfectionState::InfectedAsymptomatic}));
        indices_save_edges[1].emplace_back(
            model.populations.get_flat_index({mio::AgeGroup(i), mio::oseirvector::InfectionState::InfectedSymptomatic}));
    }

    // Build the graph
mio::Graph<mio::SimulationNode<ScalarType, mio::Simulation<ScalarType, mio::oseirvector::Model<ScalarType>>>, mio::MobilityEdge<ScalarType>> g;    
   ...
   auto make_mobility_vector = [&model](ScalarType human_rate) {
    ...
    Eigen::VectorX<ScalarType> rates = Eigen::VectorX<ScalarType>::Constant(model.populations.get_num_compartments(), human_rate);
    
    // Safely zero-lock the vector compartments specifically for Age Group 3 (Mosquitoes)
    rates[model.populations.get_flat_index({mio::AgeGroup(3), mio::oseirvector::InfectionState::Susceptible_vector})] = 0.0;
    rates[model.populations.get_flat_index({mio::AgeGroup(3), mio::oseirvector::InfectionState::Infected_vector})] = 0.0;
    ...
    auto interpolated_results_north = mio::interpolate_simulation_result<ScalarType>(results_north);
